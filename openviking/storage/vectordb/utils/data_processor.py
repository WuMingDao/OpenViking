...
class DataProcessor:
    ...
    def parse_datetime_to_epoch_ms(self, value: str) -> int:
        if not isinstance(value, str):
            raise ValueError(f"date_time value must be string, got {type(value).__name__}")
        raw = value.strip()
        if not raw:
    ...
    def parse_radius(self, value: Any) -> float:
        ...
        if raw.endswith("m"):
            num = raw[:-1].strip()
            return float(num)
        raise ValueError("radius must end with 'm' or 'km'")
